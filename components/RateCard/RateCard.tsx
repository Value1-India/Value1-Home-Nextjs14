...
export default function RateCard({ goldRate, silverRate }: RateCardProps): JSX.Element {
  ...
  return (
    <div className='w-full md:w-5/6 h-full bg-white rounded-3xl mt-24 md:mt-32 overflow-hidden border-2 md:border-4 border-white'>
      ...
      <div className='flex flex-col items-center justify-center pb-4'>{/* Buy/Sell section */}
        ...
        <div className='text-black-700 font-semibold text-xl md:text-3xl '>Enter the amount</div>
        {/* <form action="#" method="post" className='flex flex-row justify-between gap-4 items-center'> */}
        <form id="exchangeForm" action="#" method="post" className={`flex ${activeInput === 'rupees' ? 'flex-col md:flex-row' : 'flex-col-reverse md:flex-row-reverse'} justify-around md:justify-between md:gap-4 items-center px-2`}>
          ...
          <Image src={xicon} width={100} height={100} alt='exchange icon' onClick={handleExchangeClick} className='w-8 h-auto mt-4 cursor-pointer xs:rotate-90 xs:w-6 md:rotate-0 ' />
        ...
        <div className='flex flex-wrap md:flex-nowrap flex-row gap-1 md:gap-8 my-5 items-center justify-center md:justify-between'>
          <div className='w-max font-bold text-black-400 rounded-lg bg-gold-200 px-2 py-1 md:px-4 md:py-2 active:bg-gold-400 cursor-pointer' onClick={() => handleCommonButtonClick(activeInput === 'rupees' ? 100 : 1)}>
            {activeInput === 'rupees' ? '₹100' : '1g'}
          </div>
          <div className='w-max font-bold text-black-400 rounded-lg bg-gold-200 px-2 py-1 md:px-4 md:py-2 active:bg-gold-400 cursor-pointer' onClick={() => handleCommonButtonClick(activeInput === 'rupees' ? 500 : 10)}>
            {activeInput === 'rupees' ? '₹500' : '10g'}
          </div>
          <div className='w-max font-bold text-black-400 rounded-lg bg-gold-200 px-2 py-1 md:px-4 md:py-2 active:bg-gold-400 cursor-pointer' onClick={() => handleCommonButtonClick(activeInput === 'rupees' ? 1000 : 50)}>
            {activeInput === 'rupees' ? '₹1000' : '50g'}
          </div>
          <div className='w-max font-bold text-black-400 rounded-lg bg-gold-200 px-2 py-1 md:px-4 md:py-2 active:bg-gold-400 cursor-pointer' onClick={() => handleCommonButtonClick(activeInput === 'rupees' ? 5000 : 100)}>
            {activeInput === 'rupees' ? '₹5000' : '100g'}
          </div>
        </div>
        <button className='w-20 p-1 md:w-32 md:px-6 md:py-3 bg-gold-500 text-black-700 font-bold text-lg md:text-xl rounded-xl active:bg-gold-600'>
